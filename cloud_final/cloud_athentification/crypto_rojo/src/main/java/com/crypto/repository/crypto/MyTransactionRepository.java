...
public interface MyTransactionRepository extends JpaRepository<Transaction, Integer> {
    @Query("SELECT u, " +
            "COALESCE(SUM(CASE WHEN t.dateDebut <= :date THEN t.achat - t.achat*t.commission ELSE 0 END), 0) as achat, " +
            "COALESCE(SUM(CASE WHEN t.dateDebut <= :date THEN t.vente - t.achat*t.commission ELSE 0 END), 0) as vente " +
            "FROM Utilisateur u " +
